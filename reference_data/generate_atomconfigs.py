...
class AtomicData:
    def __init__(self):
        ...
        self.config = tuple()
    def __repr__(self):
        return f"{self.Z} {self.name} {self.sym} {self.config}"
...
def parse_nist_configs(ip_file: str, atoms: dict) -> None:
    ...
    #def parse_cfg_line(line: str, atom: AtomicData) -> None:
    def parse_cfg_line(line: str, atoms: dict) -> None:
        ...
        """
        try:
            Z, name, conf_s, _ = line.split()
            Z = int(Z)
            conf_s = conf_s.strip('[').split(']')  # separate core if present
            # get core config
            ccore = atoms[conf_s[0]].config if len(
